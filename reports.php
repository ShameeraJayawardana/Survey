...
if (isset($_POST['download'])){
    ...
    $header = 'District,Document ID,SD Code,Document Name,Sheet,Sup,Insert,Block,OC,FC,PP Code,Court Number,Remarks';
    //fwrite($fp, "{$header}\n");
    if (!@fwrite($fp, "{$header}\n")) {
    ...
    while ($result = mysqli_fetch_assoc($re_set)){
        $n++;
        $_result['district'] = $dis;
        $_result['doc_id'] = $doc_id;
        $_result['sd_code'] = $sd;
        $_result['doc_name'] = $doc;
        $_result['sht_no'] = $sheet;
        $_result['sup_no'] = $sup;
        $_result['inset_no'] = $insert;
        $_result['bl_no'] = $block;
        $_result['oc'] = $oc;
        $_result['fc'] = $fc;
        $_result['pp_code'] = $pp;
        $_result['court_no'] = $court;
