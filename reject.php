...
$sql = "SELECT * FROM req WHERE district = '$_row[district]' AND name = '$name' AND status = 'Rejected'";
    ...
    <?php while ($row = mysqli_fetch_assoc($row_set)) { ?>
        <div class="jumbotron">
            ...
            <span class="float-left">
                        Rejected <?php echo $row['number']; ?><br>
                        <span class="small float-right text-muted"><?php echo $row['time']; ?></span>
            </span>
        </div>
    <?php } ?>
    <a href="index.php" class="text-center text-primary">Go back to previous page</a>
</div>
</body>
</html>